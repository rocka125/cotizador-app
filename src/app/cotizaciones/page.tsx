...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const parseMXN = (v: string) => parseFloat(v.replace(/[^0-9.]/g, "")) || 0;
...
export default function CotizacionForm() {
  ...
  const rows = items.map((item) => {
    const cant = parseMXN(item.cant);
    const precio = parseMXN(item.precioUnitario);
    const descuento = parseMXN(item.descuento);
